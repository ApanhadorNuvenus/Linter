...
class YoutubeRepositoryImpl(
    ...
    override suspend fun fetchPlaybackInfo(videoId: Long, url: String, targetQuality: String): Result<VideoPlaybackInfo> = withContext(Dispatchers.IO) {
        try {
            ...
            // ИСПРАВЛЕНИЕ: Автоматически обновляем и сохраняем язык видео в БД для точной классификации СРС
            val videoEntity = videoBox[videoId]
            if (videoEntity != null && videoEntity.language != sourceLang) {
                videoEntity.language = sourceLang
                videoBox.put(videoEntity)
            }
